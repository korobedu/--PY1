...
def to_csv_file(filename: str, headers: list[str], rows: list[list[str]],
    ...
    with open(filename, 'wt') as f:
        [f.write(head+delimiter) for head in headers[:-1]]
        f.write(headers[-1]+new_line)    # Последний элемент без разделителя
        for row in rows:        # Проходим по спискам с данными
            [f.write(item+delimiter) for item in row[:-1]]
            f.write(row[-1]+new_line)    # Последний элемент без разделителя
    return
